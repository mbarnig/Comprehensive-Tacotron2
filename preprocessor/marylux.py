# created by Marco Barnig on 9.8.2021
...
from text import text_to_sequence, text2phone, phoneme_to_sequence
...
class Preprocessor:
    ...
    def process_utterance(self, raw_text, wav_path, speaker, basename):
        ...
        # Preprocess text
        print("*** marylux.py : raw_text ***")
        print(raw_text)
        text = np.array(phoneme_to_sequence(raw_text, self.cleaners, "nl"))
        print("*** marylux.py : text indices ***")
        print(text)

        # Load and process wav files
        print("**** marylux.py : wav_path ****")
        print(wav_path)
